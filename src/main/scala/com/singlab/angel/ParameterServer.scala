...
object ParameterServer {
    ...
    def startActor(driverActorPath: String) = {
        ...
        val actor = actorSystem.actorOf(Props(classOf[ParameterServerActor], remoteDriverActor), "angelParameterServerActor")

        (actorSystem, actor)
    }

    class ParameterServerActor(driver: ActorRef) extends Actor {
